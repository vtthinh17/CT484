...
class ProductsService extends FirebaseService{
  ...
  Future<List<Product>> fetchProducts([bool filterByUser = false]) async{
    final List<Product> products =[];
     try{
      ...
      final productsMap = json.decode(respone.body) as Map<String, dynamic>;
  ...
  Future<Product?> addProduct(Product product) async{
    try{
      ...
      final response = await http.post(url, body: json.encode(product.toJson()..addAll({'creatorId': userID})));
